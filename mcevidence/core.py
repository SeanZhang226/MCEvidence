...
This code requires Python 3.8 or higher.
"""
...
from mcevidence.parameter_selection import resolve_parameter_selection
from mcevidence.reporting import build_report_dict, write_report
...
__license__ = "MIT"
__version__ = "2.2.0"  # Modularized package layout, reporting and CLI improvements
...
class ChainData:
    ...
    ----------
    samples : np.ndarray
        Parameter samples from MCMC chain
    weights : np.ndarray
        Sample weights
    loglikes : np.ndarray
        Log-likelihood values
    adjusted_weights : np.ndarray
        Adjusted weights (can be modified independently)
    """
...
class SamplesMixin:
    ...
    def setup(self, str_or_dict, **kwargs):
        ...
        # For Cobaya format, use sampled parameter count from yaml if available
        # Otherwise use all columns (default behavior for CosmoMC)
        if hasattr(self, "cobaya_param_info") and self.cobaya_param_info:
            # Use number of sampled parameters (excluding derived params)
            self.nparamMC = self.cobaya_param_info["n_sampled"]
            self.logger.info(
                f"Using {self.nparamMC} sampled parameters (from Cobaya yaml)"
            )
        else:
            # Default: use all columns except weight and loglike
            self.nparamMC = self.get_shape()[1]

        # Ensure parameter names are aligned with active sampled columns.
        if self.param_names is None:
            self.param_names = [f"p{i}" for i in range(self.nparamMC)]
        else:
            self.param_names = list(self.param_names[: self.nparamMC])
    ...
    def chain_split(self, s):
        ...
        return {"s1": ChainData(s1_dict), "s2": ChainData(s2_dict)}
    ...
    def get_thin_index(self, nthin, weights):
        ...
        if nthin < 1:
            ...
            try:
                ...
                self.logger.debug(f"Weights not integers, using internal thinning: {e}")
    ...
    def thin(self, nthin=1, name=None, chain=None):
        ...
        if nthin == 1:
            # No thinning needed, return chain as-is
        ...
        try:
            ...
            self.logger.error(f"Thinning failed: {e}")
            raise
    ...
    def removeBurn(self, remove, chain=None, name=None):
        ...
        if not chain is None:
            try:
                ...
                self.logger.error(
                    f"burn-in failed: burn length {nstart} > sample length {nsamples}"
                )
                raise ValueError(
                    f"Burn-in length {nstart} exceeds chain length {nsamples}"
                ) from e
    ...
    def thin_indices(self, factor, name="s1", weights=None):
        ...
        if factor >= np.max(weights):
            ...
            tot = 0
            i = 0
            thin_ix = np.empty(norm // factor, dtype=np.int64)
            ix = 0
            mult = weights[i]
            while i < numrows:
                if mult + tot < factor:
...
class MCSamples(SamplesMixin):
    ...
    def get_param_names(self):
        """Return ordered names for the active sampled parameter columns."""
        return self.param_names

    def _get_cobaya_sampled_params(self, base_name):
        """
        Read Cobaya yaml file to get sampled parameter names and count.

        Returns
        -------
        param_info : dict
            Dictionary with 'sampled_params' (list of names),
            'n_sampled' (count including nuisance),
            'n_cosmo' (count of cosmological parameters only)
        """
        import yaml

        # Try to find .updated.yaml or .input.yaml
        yaml_file = None
        for suffix in [".updated.yaml", ".input.yaml", ".yaml"]:
        ...
        if yaml_file is None:
            self.logger.warning(f"No Cobaya yaml file found for {base_name}")
            return None
        ...
        try:
            ...
            if "params" not in config:
                self.logger.warning(f"No 'params' section in {yaml_file}")
                return None
            ...
            for param_name, param_config in config["params"].items():
                ...
                # Check if parameter is sampled (has prior)
                if isinstance(param_config, dict):
                    # Derived parameters have 'derived' key or 'value' key (fixed)
                    is_derived = param_config.get("derived", False)
                    has_value = "value" in param_config
                    has_prior = "prior" in param_config

                    if has_prior and not is_derived and not has_value:
                        sampled_params.append(param_name)

                        # Check if it's a cosmological parameter
                        # Cosmological params typically in cosmo_params_list
                        # or are common MCMC params (H0, logA, ns, ombh2, omch2, tau, etc.)
                        if param_name in cosmo_params_list or param_name in [
                            "H0",
                            "logA",
                            "As",
                            "ns",
                            ...
                            cosmo_params.append(param_name)
                        else:
                            # Likely a nuisance parameter
                            nuisance_params.append(param_name)

            self.logger.info(
                f"Found {len(sampled_params)} sampled parameters in {yaml_file}"
            )
            self.logger.info(f"  - Cosmological: {len(cosmo_params)} {cosmo_params}")
            self.logger.info(f"  - Nuisance: {len(nuisance_params)} {nuisance_params}")
            ...
            return {
                ...
                "n_nuisance": len(nuisance_params),
            }

        except Exception as e:
            self.logger.error(f"Failed to read Cobaya yaml file {yaml_file}: {e}")
            return None

    def read_list_to_array(self, flist):
        """
        Read chain files and return list of arrays.

        Handles both CosmoMC and Cobaya formats automatically.
        """
        chains = []
        for f in flist:
            self.logger.info("loading: " + f)
            # Check if file has header (Cobaya format)
            with open(f, "r") as file:
                first_line = file.readline()
                skiprows = 1 if first_line.strip().startswith("#") else 0

            # Load data skipping header if needed
            chain_data = np.loadtxt(f, skiprows=skiprows)
            chains.append(chain_data)

            if skiprows > 0:
                self.logger.debug(f"Detected Cobaya format (skipped header): {f}")

        return chains

    def _get_header_param_names(self, filename):
        """Extract parameter names from a Cobaya-style header line."""
        try:
            ...
            # Typical Cobaya columns: weight minuslogpost <params...>
            if len(tokens) > 2:
                return tokens[2:]
        except OSError:
            return None
    ...
    def _get_cosmomc_param_names(self, base_name):
        """Extract varying parameter names from a CosmoMC .ranges file."""
        ranges_file = base_name + ".ranges"
        if not os.path.exists(ranges_file):
            return None
        try:
            par = np.genfromtxt(
                ranges_file, dtype=None, names=("name", "min", "max")
            )
            names = par["name"]
    ...
    def _detect_chain_format(self, fname):
        """
        Detect if chains are in CosmoMC or Cobaya format.
        ...
        format : str
            'cobaya' or 'cosmomc'
        """
        # First check if there are actual Cobaya files (.N.txt pattern)
        if os.path.exists(fname):
            # fname is an exact file - check its pattern
            basename = os.path.basename(fname)
            if "." in basename:
                parts = basename.split(".")
                if len(parts) >= 3 and parts[-1] == "txt":
                    try:
                        int(parts[-2])  # Check if penultimate part is a number
                        return "cobaya"
                    except ValueError:
                        pass
        else:
            # fname is a base name - try to find matching files
            # Check for Cobaya pattern: basename.1.txt, basename.2.txt
            cobaya_files = glob.glob(f"{fname}.?.txt") or glob.glob(f"{fname}.??.txt")
            if cobaya_files:
                return "cobaya"

            # Check for CosmoMC pattern: basename_1.txt, basename_2.txt
            cosmomc_files = glob.glob(f"{fname}_?.txt") or glob.glob(f"{fname}_??.txt")
            if cosmomc_files:
                return "cosmomc"

        return "cosmomc"  # Default fallback

    def _get_chain_files_cobaya(self, fname):
        """
        Get list of Cobaya chain files.

        Cobaya format: basename.1.txt, basename.2.txt, etc.
        """
        # Extract base name without extension and chain number
        if fname.endswith(".txt"):
            # Remove .N.txt to get base name
            parts = fname.rsplit(".", 2)
            if len(parts) == 3:
                base = parts[0]
                # Try to find all .N.txt files
                flist = sorted(glob.glob(f"{base}.?.txt"))
                if not flist:
                    # Try two digits
                    flist = sorted(glob.glob(f"{base}.??.txt"))
                return flist

        # If pattern matching fails, try treating fname as base name
        flist = sorted(glob.glob(f"{fname}.?.txt"))
        if not flist:
            flist = sorted(glob.glob(f"{fname}.??.txt"))
    ...
    def load_from_file(self, fname, **kwargs):
        """
        Load MCMC chain files in CosmoMC or Cobaya format.

        Automatically detects format and handles both:
        - CosmoMC: basename_1.txt, basename_2.txt, ...
        - Cobaya: basename.1.txt, basename.2.txt, ...

        Parameters
        ----------
        fname : str, list
            Chain file name or list of filenames
        **kwargs : dict
            Additional arguments passed to chains2samples

        Returns
        -------
        dict
            Dictionary containing processed samples
        """

        preferred_format = kwargs.pop("chain_format", "cobaya")
        preferred_format = (preferred_format or "cobaya").lower()
        if preferred_format not in {"cobaya", "cosmomc", "auto"}:
            raise ValueError(
                f"Unsupported chain_format='{preferred_format}'. "
                "Use one of: 'cobaya', 'cosmomc', 'auto'."
            )

        # Detect format only when auto is requested, otherwise obey explicit preference.
        if preferred_format == "auto":
            chain_format = self._detect_chain_format(
                str(fname) if not isinstance(fname, (list, tuple)) else str(fname[0])
            )
            self.logger.info(f"Auto-detected chain format: {chain_format}")
        else:
            chain_format = preferred_format
            self.logger.info(f"Using requested chain format: {chain_format}")
        ...
        try:
            ...
            # if not file, try to find matching files
            if not os.path.isfile(flist[0]):
                raise FileNotFoundError(f"File not found: {flist[0]}")
            ...
            if "*" in fname or "?" in fname:
                ...
                # Try Cobaya format first
                if chain_format == "cobaya":
                    if idchain > 0:
                        flist = [f"{fname}.{idchain}.txt"]
                    else:
                        flist = self._get_chain_files_cobaya(fname)

                        if not flist:
                            self.logger.warning(
                                "No Cobaya format files found, trying CosmoMC format"
                            )
                    ...
                    # CosmoMC format
                    if idchain > 0:
                        flist = [fname + f"_{idchain}.txt"]
                    else:
                        idpattern = kwargs.pop("idpattern", "_?.txt")
                        self.logger.info(f"Loading files: {fname}{idpattern}")
                        flist = glob.glob(fname + idpattern)

                        # If CosmoMC pattern fails, try Cobaya
                        if not flist:
                            self.logger.warning(
                                "No CosmoMC format files found, trying Cobaya format"
                            )
                            flist = self._get_chain_files_cobaya(fname)

        if not flist:
            raise FileNotFoundError(f"No chain files found matching: {fname}")

        header_param_names = self._get_header_param_names(flist[0])

        # For Cobaya format, read parameter information from yaml file
        if chain_format == "cobaya":
            ...
            param_info = self._get_cobaya_sampled_params(base_name)
            if param_info:
                # Store parameter info for later use
                self.cobaya_param_info = param_info
                self.param_names = list(param_info["sampled_params"])
                # Set default nparamMC to number of sampled parameters
                # This will be used if ndim is not specified
                kwargs.setdefault("nparamMC", param_info["n_sampled"])
                self.logger.info(
                    f"Cobaya: Using {param_info['n_sampled']} sampled parameters"
                )
            else:
                self.cobaya_param_info = None
                if header_param_names:
                    self.param_names = list(header_param_names)
        else:
            self.cobaya_param_info = None
            if isinstance(fname, str):
                self.param_names = self._get_cosmomc_param_names(fname)

        try:
            # load files
            self.logger.info(f"Loading {len(flist)} chain file(s)")
            self.logger.debug("Reading from files: " + ", ".join(flist))
            self.chains = self.read_list_to_array(flist)
        except (IOError, OSError, ValueError) as e:
            self.logger.error(f"Cannot read chain from files: {flist}")
            raise IOError(f"Failed to load chain files: {flist}") from e
...
class MCEvidence:
    def __init__(
        ...
        **gdkwargs,
        ...
        gdkwargs.setdefault("chain_format", chain_format)
        #
        ...
        available_param_names = self.gd.get_param_names()
        if not available_param_names:
            available_param_names = [f"p{i}" for i in range(self.gd.nparamMC)]

        cobaya_cosmo_names = None
        if hasattr(self.gd, "cobaya_param_info") and self.gd.cobaya_param_info:
            cobaya_cosmo_names = self.gd.cobaya_param_info.get("cosmo_params", [])

        self.param_indices, self.param_names, fallback_all = resolve_parameter_selection(
            available_param_names=available_param_names,
            nparam_mc=self.gd.nparamMC,
            param_names=param_names,
            use_cosmo_params=use_cosmo_params,
            ndim=ndim,
            is_cosmo_name=iscosmo_param,
            cobaya_cosmo_names=cobaya_cosmo_names,
        )
        if fallback_all:
            self.logger.warning(
                "No cosmological parameters were identified; falling back to all sampled parameters. "
                "Use --select-params for explicit control."
            )

        self.ndim = len(self.param_indices)
        self.logger.debug("using ndim=%s" % self.ndim)
        self.logger.info("selected parameters: %s", ", ".join(self.param_names))
        #
        self.info["NparamsCosmo"] = self.ndim
        self.info["SelectedParamNames"] = ", ".join(self.param_names)
    ...
    def build_report(self, lnz_values, extra=None):
        """Build a structured run report payload."""
        return build_report_dict(
            method=self.fname if self.fname is not None else "in-memory",
            chain_format=getattr(self.gd, "chain_format", "unknown"),
            selected_param_names=self.param_names,
            nparams_mc=self.gd.nparamMC,
            ndim_used=self.ndim,
            nsamples_read=self.info.get("Nsamples_read"),
            nsamples_used=self.info.get("Nsamples"),
            prior_volume=self.priorvolume,
            lnz_values=lnz_values,
            extra=extra,
        )

    def write_report(self, path, lnz_values, report_format="json", extra=None):
        """Write run report to disk and return the written path."""
        payload = self.build_report(lnz_values=lnz_values, extra=extra)
        return write_report(path=path, payload=payload, report_format=report_format)
            ...
            if bscale == "logpower":
                ...
                for ix, nn in enumerate(self.nsample):
                    self.bsize[:, ix] = np.linspace(
                        powmin, powmax, self.nbatch, dtype=np.int64
                    )
                    self.powers[:, ix] = np.array(
                        [int(np.log10(x)) for x in self.nchain]
                    )
                self.nchain = self.bsize
    ...
    def get_samples(self, nsamples, istart=0, rand=False, name="s1", prewhiten=True):
        ...
        if prewhiten:
            ...
            try:
                ...
                self.logger.error(f"Error during chain diagonalization: {e}")
                raise RuntimeError(
                    "Failed to diagonalize chain with covariance matrix"
                ) from e
        else:
            # no diagonalisation
            Jacobian = 1
    ...
    def evidence(
        ...
        # Core estimator workflow (per batch size S):
        #   1) read samples/logL/weights
        #   2) pre-whiten samples using covariance eigendecomposition
        #   3) find k-th nearest-neighbour distances
        #   4) convert distances to n-D hypersphere volumes
        #   5) combine volumes, weights and likelihood factors into MAP(a)
        #   6) convert MAP(a) to ln(Z) via Eq. from Heavens et al. (2017)
...
def params_info(fname, cosmo=False, volumes={}):
    ...
    if glob.glob("{}*.ranges".format(fname)):
        ...
        parMax = []
        for p in par["max"]:
            p = _decode_if_bytes(p)
            parMax.append(float(p) if p != "N" else np.Infinity)
...
def get_prior_volume(args, **kwargs):
    ...
    try:
        ...
        if args.priorvolume is None:
            logger.warning(
                "Could not infer prior volume from .ranges/log.param (%s). "
                "Proceeding with prior_volume=1. "
                "For strict model comparison, provide --pvolume explicitly.",
                exc,
            )
            prior_volume = 1
        else:
...
def main(argv=None):
    """CLI entry point for evidence estimation from chain files."""
    parser = ArgumentParser(prog=sys.argv[0], add_help=True, description=desc, epilog=cite)
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help="Comma-separated parameter names to use for lnZ (e.g. 'omegabh2,omegach2,H0')",
    )
    parser.add_argument(
        "--select-params-file",
        dest="select_params_file",
        default="",
        type=str,
        help="Text file listing one parameter name per line to use for lnZ",
    )
    parser.add_argument(
        "--report-file",
        dest="report_file",
        default="",
        type=str,
        help="Optional output report path (json or txt).",
    )
    parser.add_argument(
        "--report-format",
        dest="report_format",
        default="json",
        choices=["json", "txt"],
        help="Report format when --report-file is used.",
    )
    parser.add_argument(
        "--burn",
        "--burnlen",
    ...
    parser.add_argument(
        ...
        dest="chain_format",
        default="cobaya",
        choices=["cobaya", "cosmomc", "auto"],
        help=(
            "Chain file format (default: cobaya). "
            "Use 'auto' to detect from file naming patterns."
        ),
    )
    ...
    mode_group.add_argument(
        "--allparams",
        help="Use all sampled parameters from the chain (default behavior).",
        action="store_true",
    )
    mode_group.add_argument(
        "--cosmo",
        help="Use only parameters classified as cosmological.",
        action="store_true",
    )
    ...
    if args.select_params:
        selected_params.extend(
            [p.strip() for p in args.select_params.split(",") if p.strip()]
        )
    if args.select_params_file:
        with open(args.select_params_file, "r") as fp:
            for one_line in fp:
                line = one_line.strip()
                if line and not line.startswith("#"):
                    selected_params.append(line)
    if selected_params and args.cosmo:
        parser.error("--cosmo cannot be combined with --select-params/--select-params-file")

    # get prior volume
    prior_volume = get_prior_volume(args, cosmo=args.cosmo)
    ...
    mce = MCEvidence(
        ...
        burnlen=burnlen,
        thinlen=thinlen,
        chain_format=args.chain_format,
        use_cosmo_params=args.cosmo,
        param_names=selected_params if selected_params else None,
    )
    lnz_values = mce.evidence()
    if args.report_file:
        report_path = mce.write_report(
            path=args.report_file,
            lnz_values=lnz_values,
            report_format=args.report_format,
            extra={"chain_root": method, "split_cross": split},
        )
