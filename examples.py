...
class glm_eg(object):
    def __init__(self, x=None, theta=None, rms=0.2, ptheta=None, verbose=1):
        ...
        self.theta_sample = reduce(
            np.dot, [np.linalg.inv(np.dot(self.D.T, self.D)), self.D.T, self.b]
        )
    ...
    def evidence(self):
        ...
        num2 = -0.5 * (
            np.dot(b.T, b)
            - reduce(np.dot, [b.T, D, np.linalg.inv(np.dot(D.T, D)), D.T, b])
        )
    ...
    def gibbs_dist(self, params, label):
        ...
        #
        D0 = np.zeros(shape=(ndata, 2))
        D0[:, 0] = x / sigmaNoise
        D0[:, 1] = x**2 / sigmaNoise
        D1 = np.zeros(shape=(ndata, 2))
        D1[:, 0] = 1.0 / sigmaNoise
        D1[:, 1] = x**2 / sigmaNoise
        D2 = np.zeros(shape=(ndata, 2))
        D2[:, 0] = 1.0 / sigmaNoise
        D2[:, 1] = x / sigmaNoise
    ...
    def Sampler(self, nsamples=1000):
        ...
        for i in range(Niters):
            ...
            logLikelihood[i] = -0.5 * np.dot(
                (b - np.dot(D, trace[i, :])).T, (b - np.dot(D, trace[i, :]))
            )
    ...
    def info(self):
        ...
        \n{0}-dimensional Polynomial function.""".format(
            self.ndim
        )
...
class model_2d(object):
    ...
    def lnlike(self, theta):
        m, b = theta
        model = m * self.x + b
        return -0.5 * (np.sum(((self.y - model) / self.yerr) ** 2.0))
    ...
    def vis(self, n=300, figsize=(10, 10), **kwargs):
        ...
        try:
            ...
            fig = corner.corner(self.pos(n), labels=self.label, truths=self.p, **kwargs)
...
class gaussian_eg(object):
    ...
    def pos(self, n):
        ...
        return np.random.normal(
            self.mean_sample, 5 * self.sigma_mean, size=(n, self.ndim)
        )
...
def glm_stan(iterations=10000, outdir="chains"):
    ...
    data = {"N": glmq.ndata, "K": glmq.ndim, "x": df[["x1", "x2"]], "y": glmq.y_sample}
    ...
    try:
        ...
        # and run MCMC
        fit = pystan.stan(model_code=stanmodel, data=data, iter=1000, chains=4)
        ...
        if "beta" in stan_chain.keys():
            stan_chain["samples"] = stan_chain.pop("beta")
        if "lp__" in stan_chain.keys():
            stan_chain["loglikes"] = stan_chain.pop("lp__")
...
class make_emcee_chain(object):
    ...
    def __init__(self, model, nwalkers=500, nburn=300, arg={}):
        ...
        self.emcee_sampler = emcee.EnsembleSampler(
            self.nwalkers, self.model.ndim, self.model.lnprob
        )
    ...
    def mcmc(self, nmcmc=2000, **kwargs):
        ...
        self.pos, self.prob, self.state = self.emcee_sampler.run_mcmc(
            self.pos, nmcmc, **kwargs
        )
        self.samples = self.emcee_sampler.flatchain
    ...
    def vis(self, chain=None, figsize=(10, 10), **kwargs):
        ...
        fig = corner.corner(
            chain, labels=self.model.label, truths=self.model.p, **kwargs
        )
...
def gaussian_emcee(nwalkers=300, thin=5, nmcmc=5000):
    ...
    mce = MCEvidence(chain, verbose=2, ischain=True, brange=[3, 4.2]).evidence(
        rand=True
    )
...
if __name__ == "__main__":
    ...
    if method in ["gaussian_eg", "glm_eg"]:
        ...
        mce = MCEvidence(
            chain, thinlen=2, burnlen=0.1, verbose=2, ischain=True
        ).evidence()
